...
Class Number: 282
Description:  A program that plays computer vs computer connect 4
 ************************************************************/
...
fun main() {
    ...
    while (userChoice != quit) {
        var board = Array(connectFour.screenHeight) { Array(connectFour.screenWidth) { "." } }
        ...
        if (userChoice == 1) {
            ...
            while (!playerWin) {
                ...
                println()
                printBoard(board)
                playerWin = fourInARowHorizontal("X", row, board) || fourInARowVertical("X", col, board)
                println()
                if (playerWin) {
